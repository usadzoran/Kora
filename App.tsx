...
export default function App() {
  ...
  const AdminDashboard = () => {
    ...
    // ميزانية مزامنة الإعلانات عند تغيير البيانات الأصلية
    useEffect(() => {
      setTempAds({...ads});
    }, [ads, activeTab]);
    ...
    const handleSaveAds = async () => {
      ...
      const res = await FirebaseService.updateAds(tempAds);
      if (res.success) {
        alert("تم حفظ كافة المساحات الإعلانية بنجاح.");
        fetchData(true);
      } else {
    ...
    const adKeys: { key: keyof AdConfig; label: string }[] = [
      { key: 'under_header', label: 'أسفل شريط التنقل (Header)' },
      { key: 'home_hero_bottom', label: 'أسفل واجهة الرئيسية (Hero)' },
      { key: 'hub_top', label: 'أعلى منصة الفرق' },
      { key: 'hub_bottom', label: 'أسفل منصة الفرق' },
      { key: 'matches_top', label: 'أعلى مركز المباريات' },
      { key: 'matches_bottom', label: 'أسفل مركز المباريات' },
      { key: 'live_top', label: 'أعلى صفحة البث المباشر' },
      { key: 'profile_top', label: 'أعلى ملف الفريق الشخصي' },
    ];
      ...
      <div className="max-w-7xl mx-auto py-8 md:py-12 px-4 animate-in fade-in duration-500 text-right">
        ...
        {activeTab === 'ads' && (
          <div className="bg-white p-8 md:p-12 rounded-[3.5rem] shadow-xl border border-slate-100">
              ...
              {adKeys.map(({ key, label }) => (
                <div key={key} className="space-y-3">
                  <div className="flex items-center justify-between pr-2">
                    <span className="bg-slate-100 px-4 py-1.5 rounded-full text-[10px] font-black text-slate-600 uppercase tracking-widest">{label}</span>
                  </div>
                  <textarea 
                    value={tempAds[key] || ""} 
                    onChange={e => setTempAds({...tempAds, [key]: e.target.value})}
                    className="w-full h-40 p-5 bg-slate-50 border border-slate-100 rounded-[1.5rem] font-mono text-[11px] outline-none focus:border-blue-500 focus:ring-2 ring-blue-500/10 transition-all custom-scrollbar resize-none"
                    placeholder="ضع كود HTML أو Script الإعلان هنا..."
                  />
                </div>
              ))}
            </div>
            ...
            >
              {isActionLoading ? <Loader2 className="animate-spin w-7 h-7" /> : <SaveAll className="w-7 h-7" />} 
              حفظ كافة التعديلات الإعلانية
            </button>
          </div>
        )}

        {/* بقية التبويبات (Teams, Matches, Live) تبقى كما هي */}
        {activeTab === 'teams' && (
                  ...
                  {filteredTeams.map(t => (
                    <tr key={t.id} className="group hover:bg-slate-50/50 transition-colors">
                      <td className="py-6 pr-4">
                        <div className="flex items-center gap-4">
                          <img src={t.logo_url} className="w-12 h-12 rounded-xl object-cover shadow-sm border border-slate-100" />
          ...
          <div className="space-y-12">
            <div className="bg-white p-8 md:p-12 rounded-[3.5rem] shadow-xl border border-slate-100">
               ...
               <form onSubmit={async (e) => {
                ...
                >
                  {isActionLoading ? <Loader2 className="animate-spin w-5 h-5" /> : <Save />} حفظ المباراة والجدولة
                </button>
              </form>
            </div>
          </div>
          ...
          <div className="space-y-12">
            <div className="bg-white p-8 md:p-12 rounded-[3.5rem] shadow-xl border border-slate-100">
               ...
               <form onSubmit={async (e) => {
                 ...
                 <textarea placeholder="كود التضمين HTML (Iframe)" className="w-full md:col-span-2 p-5 bg-slate-50 rounded-2xl font-mono text-xs outline-none h-32" required />
                 <button className="md:col-span-2 py-6 bg-blue-600 text-white font-black rounded-3xl shadow-xl flex items-center justify-center gap-3">
                   {isActionLoading ? <Loader2 className="animate-spin w-5 h-5" /> : <Plus />} إنشاء قناة بث
                 </button>
               </form>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto py-6 md:py-12 px-4 text-right animate-in fade-in duration-500 pb-32">
         ...
         {selectedImg && (
           ...
           >
             ...
             <button className="absolute top-6 right-6 z-10 p-3 bg-white/10 hover:bg-white/20 rounded-full text-white transition-all shadow-2xl" onClick={() => setSelectedImg(null)}><X className="w-8 h-8" /></button>
             <div className="relative max-w-full max-h-full overflow-hidden rounded-3xl md:rounded-[3rem]">
               <img src={selectedImg} className="max-w-full max-h-[85vh] object-contain select-none shadow-2xl" onClick={(e) => e.stopPropagation()} />
             </div>
           </div>
         )}
           ...
           <div className="relative z-10 p-6 md:p-12 flex flex-col md:flex-row items-center md:items-end gap-6 md:gap-10 text-white w-full">
             <div className="relative group shrink-0">
                <div className="w-32 h-32 md:w-64 md:h-64 rounded-[2rem] md:rounded-[3.5rem] border-4 md:border-8 border-white bg-white shadow-2xl overflow-hidden">
                   <img src={isEditing ? (editData.logo_url || team.logo_url) : team.logo_url} className="w-full h-full object-cover" />
                </div>
                {isEditing && isOwnProfile && (
                  <button onClick={() => logoInputRef.current?.click()} className="absolute inset-0 bg-black/60 rounded-[2rem] md:rounded-[3.5rem] flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 backdrop-blur-sm">
                    <Camera className="w-8 h-8 text-white mb-2" /><span className="text-[10px] font-black uppercase">تغيير</span>
                  </button>
                )}
             ...
             <div className="flex-1 text-center md:text-right w-full">
               <div className="flex flex-col md:flex-row items-center gap-2 md:gap-4 mb-4 justify-center md:justify-end">
                 ...
                 {isEditing ? (
                   <input value={editData.team_name} onChange={e => setEditData({...editData, team_name: e.target.value})} className="text-2xl md:text-6xl font-black italic bg-white/10 border-b-2 border-white/30 outline-none w-full max-w-sm px-2 text-center md:text-right" />
                 ) : (
               ...
               <div className="flex items-center gap-4 justify-center md:justify-end mt-4 flex-wrap">
                 <div className="flex items-center gap-2 text-blue-200 font-bold bg-white/5 px-4 py-2 rounded-xl backdrop-blur-md border border-white/10 text-sm md:text-base">
                   <MapPin className="w-4 h-4 md:w-5 md:h-5" /><span>{team.municipality || team.region}</span>
                 </div>
                 <div className="flex items-center gap-2 text-emerald-400 font-bold bg-white/5 px-4 py-2 rounded-xl backdrop-blur-md border border-white/10 text-sm md:text-base">
                   <Users className="w-4 h-4 md:w-5 md:h-5" /><span>{team.players_count || 0} لاعب</span>
                 </div>
               </div>
             </div>
             ...
             <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
               {isOwnProfile ? (
                  !isEditing ? (
                    <button onClick={startEditing} className="w-full md:w-auto bg-white text-slate-900 px-8 py-4 rounded-2xl md:rounded-[2rem] font-black text-sm md:text-lg flex items-center justify-center gap-2 hover:bg-blue-50 transition-all shadow-xl active:scale-95"><Edit3 className="w-5 h-5 md:w-6 md:h-6 text-blue-600" />إدارة الفريق</button>
                  ) : (
                    <>
                      <button onClick={saveChanges} disabled={isSaving} className="w-full md:w-auto bg-emerald-500 text-white px-8 py-4 rounded-2xl md:rounded-[2rem] font-black text-sm md:text-lg flex items-center justify-center gap-2 hover:bg-emerald-600 disabled:opacity-50 transition-all">{isSaving ? <Loader2 className="animate-spin w-5 h-5" /> : <Save className="w-5 h-5" />}حفظ</button>
                      <button onClick={() => setIsEditing(false)} className="w-full md:w-auto bg-white/10 backdrop-blur-md text-white px-8 py-4 rounded-2xl md:rounded-[2rem] font-black text-sm md:text-lg hover:bg-white/20 transition-all">إلغاء</button>
                    </>
                  )
               ) : (
                  <button onClick={handleChallengeClick} disabled={isSendingChallenge} className="w-full md:w-auto bg-gradient-to-br from-rose-600 to-orange-600 text-white px-10 py-5 rounded-[2rem] font-black text-lg md:text-xl flex items-center justify-center gap-3 hover:scale-105 transition-all shadow-2xl active:scale-95 disabled:opacity-50"><Swords className="w-7 h-7 md:w-8 md:h-8 animate-pulse" />أتحداكم!</button>
               )}
             </div>
           </div>
         </div>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
            <div className="lg:col-span-2 space-y-8 md:y-12">
              <div className="bg-white p-8 md:p-12 rounded-[2rem] md:rounded-[3.5rem] shadow-xl border border-slate-100">
                <h3 className="text-xl md:text-3xl font-black flex items-center gap-4 text-slate-900 mb-8 md:mb-12"><ImageIcon className="w-6 h-6 md:w-8 md:h-8 text-blue-600" />معرض الصور</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8">
                  {team.gallery && team.gallery.length > 0 ? (
                    team.gallery.map((img, i) => (
                      <div key={i} className="group relative aspect-square overflow-hidden rounded-[1.5rem] md:rounded-[2.5rem] shadow-lg border-2 md:border-4 border-slate-50 cursor-pointer" onClick={() => setSelectedImg(img)}>
                        <img src={img} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                        <div className="absolute inset-0 bg-blue-600/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><Maximize2 className="text-white w-8 h-8 drop-shadow-lg" /></div>
                      </div>
                    ))
                  ) : (
                    <div className="col-span-full py-16 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 text-center"><p className="text-slate-400 font-black italic">المعرض فارغ حالياً.</p></div>
                  )}
                </div>
              </div>
            </div>
  ...
  const renderContent = () => {
    if (isLoading) return <div className="min-h-screen flex flex-col items-center justify-center gap-4"><Loader2 className="w-12 h-12 text-blue-600 animate-spin" /><p className="font-bold text-slate-400">جاري تحميل البيانات...</p></div>;
    if (permissionError) return <div className="min-h-screen flex flex-col items-center justify-center gap-4 text-center px-4"><ShieldAlert className="w-16 h-16 text-red-500" /><h2 className="text-2xl font-black">خطأ في الأذونات</h2><button onClick={() => fetchData()} className="bg-blue-600 text-white px-8 py-3 rounded-2xl font-bold mt-4">إعادة المحاولة</button></div>;
        ...
        return (
          <div className="max-w-7xl mx-auto py-8 md:py-12 px-4 md:px-6 pb-24 text-right">
            ...
            <div className="text-center mb-12">
               <h2 className="text-3xl md:text-5xl font-black italic mb-2 text-slate-900 flex items-center justify-center gap-4">البث المباشر <Radio className="text-red-600 animate-pulse w-8 h-8 md:w-12 md:h-12" /></h2>
               <p className="text-slate-400 font-bold text-sm md:text-base">شاهد مباريات البطولة والبرامج الرياضية مباشرة</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {liveChannels.map(ch => (
               <div key={ch.id} className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden">
                  <div className="aspect-video relative overflow-hidden bg-slate-900">
                    <div className="w-full h-full flex items-center justify-center" dangerouslySetInnerHTML={{ __html: ch.embed_code }} />
                  </div>
                  <div className="p-8"><h4 className="font-black text-xl md:text-2xl mb-3 text-slate-800">{ch.name}</h4><p className="text-slate-500 font-bold text-sm md:text-base">{ch.description}</p></div>
               </div>
              ))}
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto py-8 md:py-12 px-4 text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
               <div className="bg-white p-8 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-xl border border-slate-100 hover:scale-105 transition-transform cursor-pointer group" onClick={() => setCurrentView('hub')}>
                  <LayoutGrid className="w-8 h-8 md:w-10 md:h-10 text-blue-600 mx-auto mb-4 md:mb-6" /><h3 className="text-lg md:text-2xl font-black mb-2">منصة الفرق</h3>
               </div>
               <div className="bg-white p-8 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-xl border border-slate-100 hover:scale-105 transition-transform cursor-pointer group" onClick={() => setCurrentView('matches')}>
                  <Calendar className="w-8 h-8 md:w-10 md:h-10 text-emerald-600 mx-auto mb-4 md:mb-6" /><h3 className="text-lg md:text-2xl font-black mb-2">المباريات</h3>
               </div>
               <div className="bg-white p-8 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-xl border border-slate-100 hover:scale-105 transition-transform cursor-pointer group" onClick={() => setCurrentView('live')}>
                  <Radio className="w-8 h-8 md:w-10 md:h-10 text-rose-600 mx-auto mb-4 md:mb-6" /><h3 className="text-lg md:text-2xl font-black mb-2">البث المباشر</h3>
               </div>
            </div>
          </div>
        );
  ...
  // المكونات الفرعية مثل LoginView, RegisterView, HubView, MatchCenterView تتبع النمط الحالي...
  const LoginView = () => (
  ...
  const MatchCenterView = () => (
    <div className="max-w-5xl mx-auto py-8 md:py-12 px-4 text-right">
      ...
      <div className="text-center mb-10 md:mb-16"><h2 className="text-3xl md:text-5xl font-black italic mb-2 text-slate-900">مركز المباريات</h2></div>
        ...
        {matches.map(match => (
          <div key={match.id} className="bg-white rounded-[2rem] md:rounded-[2.5rem] shadow-xl border border-slate-100 p-6 md:p-8 flex flex-col items-center justify-between gap-6 md:gap-8">
            <div className="flex flex-row items-center justify-between w-full">
               <div className="flex flex-col items-center gap-2 flex-1 cursor-pointer" onClick={() => { const team = allTeams.find(t => t.id === match.homeTeamId); if (team) navigateToProfile(team); }}>
                 <img src={match.homeTeamLogo} className="w-16 h-16 md:w-20 md:h-20 rounded-xl md:rounded-2xl object-cover border-2 border-slate-50 shadow-sm" /><span className="font-black text-xs md:text-xl text-slate-800 text-center">{match.homeTeamName}</span>
               </div>
               <div className="flex flex-col items-center gap-1 md:gap-2 px-4">
                 <div className="flex items-center gap-2 md:gap-4 bg-slate-900 text-white px-4 md:px-8 py-2 md:py-4 rounded-xl md:rounded-[2rem] shadow-lg"><span className="text-2xl md:text-4xl font-black">{match.scoreHome}</span><span className="text-slate-500 font-black text-xl md:text-2xl">-</span><span className="text-2xl md:text-4xl font-black">{match.scoreAway}</span></div>
               </div>
               <div className="flex flex-col items-center gap-2 flex-1 cursor-pointer" onClick={() => { const team = allTeams.find(t => t.id === match.awayTeamId); if (team) navigateToProfile(team); }}>
                 <img src={match.awayTeamLogo} className="w-16 h-16 md:w-20 md:h-20 rounded-xl md:rounded-2xl object-cover border-2 border-slate-50 shadow-sm" /><span className="font-black text-xs md:text-xl text-slate-800 text-center">{match.awayTeamName}</span>
               </div>
            </div>
          </div>
        ))}
      ...
      <div className="max-w-4xl mx-auto py-8 md:py-12 px-4 pb-24 text-right">
         ...
         {currentUser && (
           <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] shadow-xl border border-slate-100 p-6 md:p-8 mb-10">
             ...
             <textarea value={newPostContent} onChange={e => setNewPostContent(e.target.value)} placeholder="ماذا يدور في كواليس فريقك اليوم؟" className="w-full h-24 md:h-32 p-4 md:p-5 bg-slate-50 border border-slate-100 rounded-[1.5rem] md:rounded-[2rem] outline-none font-bold text-right text-slate-800 text-sm md:text-base" />
             <div className="flex justify-start mt-4">
                <button onClick={handleCreatePost} disabled={isPosting || !newPostContent.trim()} className="bg-blue-600 text-white px-8 py-3 rounded-xl md:rounded-2xl font-black flex items-center gap-2 hover:bg-blue-700 transition-all disabled:opacity-50 text-sm md:text-base">
               ...
               <div key={post.id} className="bg-white rounded-[2rem] md:rounded-[2.5rem] shadow-xl border border-slate-100 p-6 md:p-10 text-right">
                  <div className="flex items-center gap-4 md:gap-5 mb-6 md:mb-8 justify-end">
                    <div className="text-right">
                       <h4 className="font-black text-base md:text-xl text-slate-900 cursor-pointer hover:text-blue-600" onClick={() => { const team = allTeams.find(t => t.id === post.teamId); if (team) navigateToProfile(team); }}>{post.teamName}</h4>
                  ...
                  <div className="flex items-center gap-4 md:gap-6 border-t border-slate-50 pt-4 md:pt-6">
                     <button onClick={() => handleLike(post)} disabled={!currentUser} className={`flex items-center gap-1 md:gap-2 font-black transition-colors text-sm md:text-base ${isLiked ? 'text-rose-500' : 'text-slate-400'}`}>
                        <Heart className={`w-5 h-5 md:w-6 md:h-6 ${isLiked ? 'fill-current' : ''}`} /><span>{post.likes?.length || 0}</span>
                     </button>
                     <button onClick={() => setActiveCommentPostId(activeCommentPostId === post.id ? null : post.id!)} className={`flex items-center gap-1 md:gap-2 font-black transition-colors text-sm md:text-base ${activeCommentPostId === post.id ? 'text-blue-600' : 'text-slate-400'}`}>
                        <MessageSquare className="w-5 h-5 md:w-6 md:h-6" /><span>{post.comments?.length || 0}</span>
                     </button>
                  </div>
               </div>
             );
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4 md:gap-8">
            <div className="flex items-center gap-2 cursor-pointer group" onClick={() => { setCurrentView('home'); setViewedTeam(null); }}>
               <div className="bg-blue-600 p-1.5 md:p-2 rounded-xl md:rounded-2xl shadow-lg"><Trophy className="w-4 h-4 md:w-6 md:h-6 text-white" /></div>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            {currentUser && (
               <div className="relative" ref={notifRef}>
                  <button onClick={() => setIsNotifMenuOpen(!isNotifMenuOpen)} className="relative p-2 md:p-3 bg-slate-50 rounded-xl md:rounded-2xl border border-slate-100 text-slate-600 hover:text-blue-600 transition-colors">
                    <Bell className="w-5 h-5 md:w-6 md:h-6" />
                    {unreadCount > 0 && <span className="absolute -top-1 -left-1 w-5 h-5 md:w-6 md:h-6 bg-rose-500 text-white text-[9px] md:text-[10px] font-black flex items-center justify-center rounded-full border-2 border-white animate-pulse">{unreadCount}</span>}
                  </button>
                  {isNotifMenuOpen && (
                    <div className="absolute left-0 mt-3 w-72 md:w-80 bg-white rounded-3xl shadow-2xl border border-slate-100 py-4 z-[60] animate-in slide-in-from-top-2">
                       <div className="px-6 pb-4 border-b border-slate-50 flex items-center justify-between"><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">الإشعارات</span></div>
                       <div className="max-h-96 overflow-y-auto custom-scrollbar">
                          {notifications.length === 0 ? <div className="py-12 text-center text-slate-400 font-bold italic text-sm">لا توجد إشعارات حالياً.</div> : notifications.map(n => (
                              <div key={n.id} onClick={() => { FirebaseService.markNotificationRead(n.id!); if(n.type === 'challenge') navigateToProfile(currentUser); setIsNotifMenuOpen(false); }} className={`px-6 py-4 border-b border-slate-50 last:border-none cursor-pointer transition-colors ${n.isRead ? 'opacity-60' : 'bg-blue-50/30'}`}>
                                 <div className="flex gap-3 items-start">
                                    <div className={`p-2 rounded-lg ${n.type === 'challenge' ? 'bg-rose-100 text-rose-600' : 'bg-blue-100 text-blue-600'}`}>{n.type === 'challenge' ? <Flame className="w-4 h-4" /> : <Bell className="w-4 h-4" />}</div>
                                    <div className="flex-1"><div className="text-sm font-black text-slate-800 mb-0.5">{n.title}</div><div className="text-xs font-medium text-slate-500 leading-tight">{n.message}</div></div>
                                 </div>
                              </div>
                            ))
                          }
               ...
               <div className="relative" ref={menuRef}>
                  ...
                  {isUserMenuOpen && (
                    <div className="absolute left-0 mt-3 w-48 md:w-64 bg-white rounded-2xl md:rounded-3xl shadow-2xl border border-slate-100 py-2 md:py-3 z-[60]">
                       <button onClick={() => navigateToProfile(currentUser)} className="w-full flex items-center justify-end gap-3 px-4 md:px-6 py-2 md:py-3 hover:bg-slate-50"><span className="font-bold text-slate-700 text-xs md:text-sm">الملف الشخصي</span><User className="w-4 h-4 md:w-5 md:h-5 text-blue-600" /></button>
                       <div className="h-px bg-slate-100 my-1 md:my-2 mx-4 md:mx-6" />
                       <button onClick={handleLogout} className="w-full flex items-center justify-end gap-3 px-4 md:px-6 py-2 md:py-3 hover:bg-red-50 text-red-500"><span className="font-bold text-xs md:text-sm">تسجيل الخروج</span><LogOut className="w-4 h-4 md:w-5 md:h-5" /></button>
                    </div>
                  )}
               </div>
            ) : (
